...
object ContentObserverManager {
    ...
    /**
     * 触发全屏通知，通过FullScreenIntent唤起AlertActivity
     * 适配Android 10+的后台启动Activity限制
     */
    private fun triggerFullScreenNotification(context: Context, sender: String, message: String) {


        FwLog.d("短信变化: startFloatWindow")
            ...
            val observer = object : ContentObserver(handler) {
                override fun onChange(selfChange: Boolean, uri: Uri?) {
                    ...
                    val cursor = context.contentResolver.query(
                        Telephony.Sms.Inbox.CONTENT_URI, // 收件箱URI
                        arrayOf(Telephony.Sms.BODY, Telephony.Sms.ADDRESS), // 要查询的列：短信正文、发件人
                        null,
                        null,
                        "${Telephony.Sms.DATE} DESC LIMIT 1" // 按时间倒序，取最新1条
                    )
